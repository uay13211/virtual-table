...
export const VirtualTable = memo(function <RowType extends object>({
  columns,
  rowHeight,
  dataSource,
  className,
  ...
  const transformedColumns = useRowSelection({
    columns,
    dataSource,
    rowKey,
    rowSelection,
  ...
  const { headerRef, getHeaderRef, columnWidths, calcColumnWidths } =
    useColumnWidths();
  ...
  const columnsStickyPosition = useColumnsStickyPosition(
    transformedColumns,
    columnWidths
  );
  ...
  const containerHeight = scrollY
    ? scrollY + headerHeight + (scrollable.horizontal ? scrollBarSize : 0)
    : "100%";

  return (
    <div
      className={classNames("g-virtual-table", className)}
      style={{ width: scrollX || "100%", height: containerHeight }}
    >
