...
export const TableRow = memo(function <RowType extends object>({
  virtualItem,
  data,
  columns,
  columnWidths,
  ...
  const lastShownColumnIndex: number = React.useMemo(
    () => columns.findLastIndex((_) => _.display !== "hidden"),
    [columns]
  );
    ...
    >
      {columns.map((column, columnIndex) => {
        const colSpan = column.colSpan
          ? column.colSpan(data, rowIndex, columnIndex)
          : 1;
        // handle colspan > 1
        const cellWidth =
          colSpan > 1
        ...
        const renderData =
          column.display !== "hidden" && column.renderData(data, rowIndex);
          ...
          renderData && (
            <div
              ...
              style={{
                ...
                left:
                  column.fixed === "left" ? stickyPosition?.value : undefined,
                right:
                  column.fixed === "right"
                    ? stickyPosition?.value -
                      (isLastShownColumn ? 0 : scrollBarSize)
                    : undefined,
              }}
            >
              {renderData}
