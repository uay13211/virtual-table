...
export const useColumnsStickyPosition = <RowType extends object>(
  ...
  return React.useMemo(() => {
    ...
    const leftFixedCols = ArrayUtil.compactMap(columns, (_, columnIndex) =>
      _.fixed === "left"
        ? { columnIndex, width: columnWidths[columnIndex] || 0 }
        : null
    );
    // the right sticky value stack in reverse direction
    const rightFixedCols = ArrayUtil.compactMap(columns, (_, columnIndex) =>
      _.fixed === "right"
        ? { columnIndex, width: columnWidths[columnIndex] || 0 }
        : null
    ).reverse();
    ...
    leftFixedCols.forEach((column, idx) => {
      columnsStickyPosition[column.columnIndex] = {
        value: leftColumnsStackPositionValue,
        isLast: idx === leftFixedCols.length - 1,
      };
    ...
    rightFixedCols.forEach((column, idx) => {
      columnsStickyPosition[column.columnIndex] = {
        value: rightColumnsStackPositionValue,
        isLast: idx === rightFixedCols.length - 1,
      };
