...
export const useRowSelection = function <RowType extends object>({
  columns,
  dataSource,
  rowSelection,
  rowKey,
  ...
  const transformedColumns = React.useMemo(() => {
    ...
    const onSelectAll = (val: boolean) => {
      return val
        ? onChange(allSelectionRowKeys, allSelectionRows)
        : onChange(unAllSelectionRowKeys, unAllSelectionRows);
    };

    const isAllSelectionDisabled =
      isSelectAllDisabled || enabledRowKeys.length === 0;
    const isAllSelected =
      enabledCheckedRowKeys.length > 0 &&
      enabledRowKeys.length === enabledCheckedRowKeys.length;
    ...
    const rowSelectionColumn: VirtualTableColumn<RowType> = {
      ...
      renderData: (data, rowIndex) => {
        ...
        const toggledSelectedRowKeys = ArrayUtil.toggleElement(
          selectedRowKeys,
          key
        );
        ...
        return (
          <Checkbox
            ...
            value={isChecked}
            onChange={() =>
              onChange(toggledSelectedRowKeys, toggledSelectedRow)
            }
          />
        );
      },
